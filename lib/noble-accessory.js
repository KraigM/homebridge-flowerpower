...
module.exports = function (_Accessory, _Service, _Characteristic, _uuid) {
	if (_Accessory) {
		...
		NobleDeviceAccessory.prototype.runAsync = runAsync;
...
function NobleDeviceAccessory(deviceId, log) {
...
var locate = function () {
	return new Promise(function (resolve, reject) {
		...
		NobleDevice.discoverById(discover);
	});
};

var initAsync = function(device) {
	return this.runAsync(this.updateInitialDataAsync.bind(this), device);
};

var runAsync = function(func, device) {
	if (this.device && this.device.connectedAndSetUp) {
		return func();
	} else {
		var disconnect = this.disconnectAsync.bind(this);
		return this.connectAsync(device, 3)
			.then(func)
...
var updateDataAsync = function(prop, characteristic, service, format) {
	var self = this;
	return new Promise(function(resolve, reject){
		var fn = 'read' + prop;
		if (!self.device[fn]) {
